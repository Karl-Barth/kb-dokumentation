...
Der Status-Wert in der Datenbank (z.B. «Entwurf») wird beim Export automatisch in das `@status`-Attribut des `<bibl>`-Elements übernommen; er steuert nur die Anzeige in der automatisch generierten Bibliografie.
